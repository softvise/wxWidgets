...
public:
    ...
    struct DropItemInfo
    {
        ...
        ,   m_dropHint(wxDND_DROP_HINT_NONE)
        ,   m_item(nullptr)
...
void wxDataViewMainWindow::RefreshDropHint()
{
    ...
    if ((m_dropItemInfo.m_dropHint & wxDND_DROP_HINT_ABOVE) != 0)
        RefreshRows(row == 0 ? row : row-1, row);
    else if ((m_dropItemInfo.m_dropHint & wxDND_DROP_HINT_BELOW) != 0)
        // It's not a problem here if endRow+1 is out of range, RefreshRows() allows this.
        RefreshRows(row, row+1);
    else if ((m_dropItemInfo.m_dropHint & wxDND_DROP_HINT_INSIDE) != 0)
        RefreshRow(row);
}
...
wxDataViewMainWindow::DropItemInfo wxDataViewMainWindow::GetDropItemInfo(const wxCoord x, const wxCoord y)
{
    ...
    const bool isVirtualList = IsVirtualList();
    ...
    const bool useExtendedDropHint = GetOwner()->HasFlag(wxDV_EXTENDED_DROP_HINT);

    // Set the drop item hint:
    //   If wxDV_EXTENDED_DROP_HINT is set:
    ...
    const int yDistanceFromLineStart = yy - GetLineStart(row);
    const int itemHeight = GetLineHeight(row);

    const bool insertAbove = yDistanceFromLineStart < itemHeight / 2;
    if (insertAbove)
        dropItemInfo.m_dropHint = wxDND_DROP_HINT_ABOVE;
    else
    ...
    const bool insertInside = yDistanceFromLineStart > borderDropHintHeight &&
                              yDistanceFromLineStart <= borderDropHintHeight + insideDropHintHeight;
    if (insertInside)
    {
        if (useExtendedDropHint)
        {
            dropItemInfo.m_dropHint |= wxDND_DROP_HINT_INSIDE;
        }
        else
        {
            dropItemInfo.m_dropHint = wxDND_DROP_HINT_INSIDE;
    ...
    if ((dropItemInfo.m_dropHint & wxDND_DROP_HINT_ABOVE) != 0)
    {
                                               ...
                                               wxCoord y, wxDragResult def )
{
    ...
    event.SetActiveDropIndex(nextDropItemInfo.m_row);
    ...
    if (nextDropItemInfo.m_dropHint != wxDND_DROP_HINT_NONE)
    {
...
bool wxDataViewMainWindow::OnDrop( wxDataFormat format, wxCoord x, wxCoord y )
{
    ...
    event.SetActiveDropIndex(dropItemInfo.m_row);
...
void wxDataViewMainWindow::OnPaint( wxPaintEvent &WXUNUSED(event) )
{
...
#if wxUSE_DRAG_AND_DROP
    // Determine the height of the drop above or drop below hint.
    int borderDropHintHeight = 0;
    if (m_dropItemInfo.m_row >= 0)
    {
        const int dropItemHeight = GetLineHeight(m_dropItemInfo.m_row);
        borderDropHintHeight = makeBorderDropHintHeight(dropItemHeight);
    }
#endif // wxUSE_DRAG_AND_DROP
    ...
    for (unsigned int i = col_start; i < col_last; i++)
    {
        ...
        for (unsigned int item = item_start; item < item_last; item++)
        {
            ...
            int dropHint = wxDND_DROP_HINT_NONE;
            if (item == m_dropItemInfo.m_row)
            {
                // Draw the drop hint for the item as given by m_dropItemInfo.
                dropHint = m_dropItemInfo.m_dropHint;
            }
            else if (item == m_dropItemInfo.m_row + 1 && 
                     m_dropItemInfo.m_dropHint == wxDND_DROP_HINT_BELOW)
            {
                // In case the drop hint is "below" we additionally draw a drop above hint for
                // the item below. This way, the z-order of drawing items and hints remains correct.
                dropHint = wxDND_DROP_HINT_ABOVE;
            }
            else if (item + 1 == m_dropItemInfo.m_row &&
                     m_dropItemInfo.m_dropHint == wxDND_DROP_HINT_ABOVE)
            {
                // In case the drop hint is "above" we additionally draw a drop below hint for
                // the item above. This way, the z-order of drawing items and hints remains correct.
                dropHint = wxDND_DROP_HINT_BELOW;
            }
#endif // wxUSE_DRAG_AND_DROP
            ...
            if ((!IsList()) && (col == expander))
            {
                ...
                if ( node->HasChildren() )
                {
                    ...
                    renderer.DrawTreeItemButton( this, dc, rect, flag);
                }
                ...
                if (dropHint == wxDND_DROP_HINT_ABOVE || dropHint == wxDND_DROP_HINT_BELOW)
                {
...
#if wxUSE_DRAG_AND_DROP
            // Draw the drop inside hint underneath the cell's content
...
#if wxUSE_DRAG_AND_DROP
            // Draw the drop above hint or the drop below hint over the cell's content.
